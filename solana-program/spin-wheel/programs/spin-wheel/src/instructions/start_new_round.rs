use crate::{
    ErrorCode, GamePotSol, GameState, PlayerData, RoundState, SeedArray, MAX_PLAYERS,
    MAX_ROUND_DURATION, MIN_ROUND_DURATION, SEED_BYTES_LENGTH,
};
...
pub fn process_start_new_round(
    ...
    ); // Key is available after init by Anchor
    msg!(
        "GamePotSol PDA to be initialized at: {}",
        ctx.accounts.game_pot.key()
    );
    msg!(
        "Seed Commitment (first 5 bytes): {:?}",
        &seed_commitment[0..5]
    );
    ...
    round_state.players = [PlayerData::default(); MAX_PLAYERS];
    ...
    msg!("  Is Active: {}", round_state.is_active);
    msg!(
